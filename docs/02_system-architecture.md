...
2. **Backend API (Hono + Node.js)**: Uma API extremamente leve e veloz rodando com suporte extensível a webhooks e streaming hospedada na **Vercel**. Usa Zod para validação end-to-end (RPC / OpenAPI). Todo tráfego precisa enviar o cabeçalho `x-tenant-id` garantindo isolamento de dados.
...
5. **AI/ML Layer (@leadgers/ai)**: Integração centralizada com LLMs (nativamente **Anthropic Claude 3.5/3.7** via adaptadores, conforme PRD v1.1).
...
- **Funcionamento Mecânico**:
  ...
  - O `ClaudeAdapter` (LLM) é injetado. O texto é gerado numa linguagem executiva e analítica apontando gargalos financeiros.
...
Todo Roteamento (`app.ts`) abstrai a repetição de códigos de segurança de tenant e RLS da seguinte forma generalista, utilizando padrão Middleware:
...
const roadmapRoutes = new Hono()
  ...
  .post("/import-github", validateBody(mySchema), async (c) => {
    // Escopo seguro: Protegido pro 'tenant atual' e Payload 100% tipado por Zod.
    // O Prisma Client atua como Singleton para prevenir exaustão de conexões.
  });
// Nota: O arquivo principal aplica Global Rate Limiting in-memory evitando ataques Flood.
```
