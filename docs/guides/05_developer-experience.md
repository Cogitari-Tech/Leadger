...
*Note: Both are injected directly into `apps/web/src/main.tsx`. There is no need to manually inject them inside page components.*
...
If an E2E test runs a signup with an *already existing email*, Supabase Auth will **return a success packet without a session** to avoid Email Enumeration attacks (OWASP Best Practice). 
...
When performing E2E tests, deleting users row-by-row via standard ORM or scripts usually fails due to Postgres constraint triggers (e.g., *a tenant must have at least one owner*). 
