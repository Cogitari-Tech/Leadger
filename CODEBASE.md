...
1. **Repositories**: Implements core interfaces using the Supabase client.
   - *Example:* `src/modules/audit/repositories/SupabaseAuditRepository.ts` implements `IAuditRepository`.
2. **State & Hooks**: Local state management (Zustand, React hooks) relies on core entities.
   - *Example:* `useFinance.ts` and `financeStore.ts` handle `Transaction` and `Account` entities.
...
1. **Adapters**: Implements core interfaces using the Prisma ORM.
   - *Example:* `src/adapters/PrismaFinanceRepository.ts` implements `IFinanceRepository`.
